import { defineComponent, ref, watch, onMounted, nextTick, onBeforeUnmount, createElementBlock, openBlock, createElementVNode } from "vue";
...
function loadSelect2(callback) {
  ...
  script.onerror = () => {
    console.error("[Select2Vue3] Failed to load Select2");
  };
...
const _sfc_main = defineComponent({
  ...
  props: {
    ...
    name: { type: String, default: "select2" },
    ...
    loadingSelect: { type: Boolean, default: void 0 }
  },
  emits: [
    "update:modelValue",
    "change",
  ...
  setup(props, { emit }) {
    ...
    let latestSuccessCallback = null;
      ...
      if (props.fetchOnSearch) {
        baseConfig.ajax = {
          ...
          transport: (params, success, failure) => {
            ...
            latestSuccessCallback = success;
            emit("search", term, page);
            const waitUntilReady = (retry = 0) => {
              const isReady = props.loadingSelect !== void 0 ? props.loadingSelect === false : props.options.length > 0;
              if (isReady) {
                if (latestSuccessCallback) {
                  latestSuccessCallback({
                    results: props.options,
                    pagination: {
                      more: props.config?.hasMore ?? false
                    }
                  });
                  latestSuccessCallback = null;
                }
                isFetching.value = false;
              } else if (retry < 100) {
                setTimeout(() => waitUntilReady(retry + 1), 10);
              } else {
                console.warn("[Select2] Max retry reached");
                failure?.();
                isFetching.value = false;
              }
            };
    ...
    const bindEvents = ($el) => {
      ...
      nativeEvents.forEach(
        (event) => $el.on(`select2:${event}`, (e) => {
          emit(event, e.params?.data || e);
        })
      );
      $el.on("select2:open", () => {
        attachScroll();
    ...
    const attachScroll = () => {
      const $results = $(selectElement.value).data("select2")?.$results;
      if ($results && $results.length) {
        $results.off("scroll", onScroll);
    ...
    const onScroll = () => {
      const $results = $(selectElement.value).data("select2")?.$results;
      if (!$results || !$results.length || isScrollLoading.value) return;
      const scrollTop = $results.scrollTop();
      const innerHeight = $results.innerHeight();
      const scrollHeight = $results[0].scrollHeight;
      if (scrollTop + innerHeight >= scrollHeight - 30) {
        if (props.config?.hasMore !== false) {
    ...
    const destroySelect2 = () => {
      if (selectElement.value && $.fn?.select2) {
        $(selectElement.value).off().select2("destroy");
      }
    };
    const initSelect2 = () => {
      const $el = $(selectElement.value);
      $el.select2(buildConfig());
    ...
    watch(() => props.modelValue, (val) => {
      if (selectElement.value && $.fn?.select2) {
        ...
        $(selectElement.value).val(val).trigger("change");
    ...
    watch(
      ...
      (newOptions, oldOptions) => {
        const newlyAppended = newOptions.slice(oldOptions.length);
        if (props.fetchOnSearch && newlyAppended.length > 0 && typeof latestSuccessCallback === "function") {
          latestSuccessCallback({
            results: newlyAppended,
            pagination: {
              more: props.config?.hasMore ?? false
            }
          });
...
const _hoisted_2 = ["id", "name", "multiple", "disabled", "required", "placeholder"];
...
const Select2Vue3 = /* @__PURE__ */ _export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-a5ad395c"]]);
const index = {
  install(app) {
